...
def test_local_adapter_supports_czech() -> None:
    ...
    assert sentence.sentence == "Můj bratr chce zítra používat."
    assert translation.translation == "My brother wants to use tomorrow."
...
def test_local_adapter_supports_japanese() -> None:
    sentence = LocalSentenceAdapter().generate_sentence(
        SentenceGenerationRequest(
            display_form="使う",
            lemma="使う",
    ...
    assert sentence.sentence == "兄は明日使うつもりです。"
    assert translation.translation == "My brother wants to use tomorrow."
