...
class KoreanFoundationCurationRecord(_FrozenReviewModel):
    ...
    def identity_and_gate_set_must_be_deterministic(self) -> Self:
        ...
        if self.source_pack_version != expected_version:
...
def _build_pending_korean_foundation_curation(
    *,
    ...
    payload: dict[str, object] = {
        "schema_version": 1,
        "manifest_version": "korean-foundations-v1-curation",
        "candidate_only": True,
...
def load_pending_korean_foundation_curation() -> KoreanFoundationCurationManifest:
    """Load only the fixed top-level candidate and prove it remains source-aligned."""

    manifest = _load_curation_manifest(DEFAULT_KOREAN_FOUNDATION_CURATION_PATH)
