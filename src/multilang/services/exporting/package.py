...
def build_multilang_note(row: ExportCardRow, *, model: genanki.Model | None = None) -> genanki.Note:
    ...
    note = MultilangNote(
        ...
        # New grammar cards must retain their prerequisite order on import.
        due=(row.identity.sort_index
             if row.identity.language == SupportedLanguage.KO
             and row.identity.source_type == "korean-grammar" else 0),
    )
