...
- **Active Phase:** Phase 31 - Hangul and Pronunciation i+1 (replanned for assisted curation; Plan 31-21 complete; Phase 31 remains open).
- **Last Completed:** Plan 31-21 - published the exact selected four-member v2 candidate bundle through a four-field atomic `current-candidate.json` pointer with RED/GREEN coverage, fixed promote/verify CLI commands, idempotent retry verification, no provider calls, no approvals, no v1 mutation, and no canonical evidence/export mutation on 2026-08-24.
