# Requirements archive: v3.0 Korean implementation

**Archived:** 2026-09-19. **Scope:** code and tests, `repo_closeout`.
**Implementation coverage:** 32/32; original production/delivery acceptance
is **not all satisfied**. The table separates implemented contracts from the
unchanged original specification reproduced below. See
`../KOREAN-PRODUCTION-BACKLOG.md` for outstanding acceptance.
...
## Original specification before closeout — preserved verbatim

The original checkboxes, Done-When clauses and failed pilot history below are
historical facts, not current implementation-state markers. They are retained
so that final production acceptance can be verified against the original goals.
...
- **Plan 32-54 Superseding Update:** Plan 32-54 is BLOCKED/FAILED partial evidence, not completion evidence. Secret-safe preflight passed at Alembic head `20260828_19`; a separate `pilot_base` job was persisted with exactly 20 deterministic level-1 candidates and `total_items=20`. Unit 1 created zero text records after exactly two external attempts received the controlled `rate_limited` classification. Three provider telemetry rows represent those two attempt rows plus one outer `ProviderRetryError` aggregate failure row for the same controlled identity. Unit 2 was not attempted. Accepted, review-required, audio, Azure, card-export, deck-export, and full-run counts are all zero. No requirement or ROADMAP criterion is newly satisfied, and Phase 33 remains unchanged.
- **Plan 32-54 Blocker:** Automatic same-job replay is forbidden because provider-attempt history now exists. A newly checked recovery plan and fresh paid-call approval are required before retrying Unit 1. Unit 2, any 3000-item operation, Azure/audio, review, export, release, publication, delivery, Git action, and Phase 32 closure remain unauthorized.
- **Plan 32-54 Next:** Ask whether the user authorizes a newly checked same-job recovery plan to retry only Unit 1 after provider quota/rate-limit readiness is confirmed, retaining the same two-attempt ceiling and permitting Unit 2 only after exactly 10 text records exist.

- **Plan 32-55 Superseding Update:** The user's `Autorizar retry` decision was consumed by exactly one contained same-job recovery invocation. Test-first offline gates and the production recovery preflight passed from the exact immutable Plan 32-54 state after the provider cooldown. The one-shot marker was created before provider construction, but the invocation ended in machine-valid `bounded_refailure` before any new external provider-attempt row or text record was created. The job still has exactly 20 original candidates, zero text/audio/export rows, the original authority binding, and the three immutable Plan 32-54 telemetry rows. Unit 2 was not attempted.
- **Plan 32-55 Blocker:** The single recovery authority is exhausted. Independent `gsd-verifier` passed evidence-integrity and boundary checks, but found the bounded refailure was caused by a local helper bug: `build_runtime()` did not return the constructed runtime, so the recovery marker was consumed before provider construction. No additional retry, replacement candidate, intermediate pilot, audio/Azure action, review, export, release, publication, delivery, Git action, 3000-item execution, requirement completion, or Phase 32 closure is authorized.
- **Plan 32-55 Next:** Create a narrow offline fix plan for the helper regression and tests before any future paid retry. Any later provider retry requires a new explicit user decision and a newly checked plan; do not reuse the consumed marker or authority.
